...
def chart_NEW1(df: pd.DataFrame) -> alt.Chart: 
    return (
        alt.Chart(df)
        .mark_line()
        .encode(
            x=alt.X("date:T", title="Date"),
            y=alt.Y("precipitation:Q", title="Daily precipitation (in)"),
            tooltip=[alt.Tooltip("date:T"), alt.Tooltip("precipitation:Q", format=".1f")],
        )
        .properties(height=320)
    )

def chart_NEW2(df: pd.DataFrame) -> alt.Chart: 
    month_order = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
    return (
        alt.Chart(df).mark_errorbar(extent="ci", ticks=True, color = "blue")
        .encode(
            x=alt.X("month_name:N", title="Month", sort=month_order),
